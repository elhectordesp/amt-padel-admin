...
export default async function TorneoPublicoPage({ params }: { params: { id: string } }) {
  ...
  // Cargar grupos de todas las categorías en paralelo
  const groupsByCategory = await Promise.all(
    categories.map(async (cat: any) => ({
      cat,
      groups: await getGroups(params.id, cat.id),
    }))
  );
  ...
  return (
    <div className="min-h-screen bg-[#0C0C0C] text-white">
      ...
      <main className="max-w-4xl mx-auto px-4 py-10 space-y-10">
        ...
        {groupsByCategory.map(({ cat, groups }) => (
